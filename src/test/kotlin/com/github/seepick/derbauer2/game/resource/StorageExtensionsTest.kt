...
class StorageExtensionsTest : DescribeSpec({
    ...
    describe("isAbleToStore") {
        it("Given storage Then yes") {
            user.addAndSet(Granary(), 10.z)
            val food = user.addAndSet(Food(), 0.z)
        ...
        it("Given full storage Then no") {
            val storage = user.addAndSet(Granary(), 10.z)
            val food = user.addAndSet(Food(), storage.totalStorageAmount)
