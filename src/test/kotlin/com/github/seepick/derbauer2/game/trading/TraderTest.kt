    ...
    context("Buy") {
        describe("Results") {
            it("When buy infinite resource like gold Then succeed") {
                ...
                ) shouldBeEqual UserResult.Success
            }
            ...
            it("Given no storage When buy Then fail") {
                ...
                user.trade(
                    TradeRequest(Food::class, Buy, 1.units),
                ).shouldBeInstanceOf<UserResult.Fail.InsufficientResources>()
            }
        }
        ...
        describe("Operation") {
            ...
            it("When buy infinite resource gold Then increased") {
                user.addEntity(Gold(0.units))
                user.trade(TradeRequest(Gold::class, Buy, 1.units))
    ...
    context("Sell") {
        ...
        describe("Operation") {
            it("Given no gold When selling Then gold unchanged") {
                user.addEntity(Gold(0.units))
                user.trade(TradeRequest(Gold::class, Sell, 1.units))
